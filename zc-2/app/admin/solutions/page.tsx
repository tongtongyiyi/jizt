...
export default function SolutionsPage() {
  ...
  const [form, setForm] = useState({ title: "", description: "", icon: "", sortOrder: 0, isActive: true });
  ...
  function startCreate() {
    setEditing({ id: 0, title: "", description: "", icon: "", sortOrder: 0, isActive: true });
    setForm({ title: "", description: "", icon: "", sortOrder: 0, isActive: true });
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const url = editing && editing.id > 0 ? `/api/solutions/${editing.id}` : "/api/solutions";
    ...
    <div>
      ...
      {editing && (
        <form onSubmit={handleSubmit} className="mt-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold">{editing.id > 0 ? "编辑方案" : "新增方案"}</h2>
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">图标标识</label>
              <input className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" value={form.icon} onChange={e => setForm({ ...form, icon: e.target.value })} placeholder="例如: water, traffic" />
            </div>
            ...
            {solutions.map(s => (
              <tr key={s.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${s.isActive ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-600"}`}>
                    {s.isActive ? "上架" : "下架"}
                  </span>
                </td>
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => startEdit(s)} className="rounded p-1 text-gray-500 hover:bg-gray-100 hover:text-blue-600"><Pencil size={16} /></button>
                    <button onClick={() => handleDelete(s.id)} className="rounded p-1 text-gray-500 hover:bg-gray-100 hover:text-red-600"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))}
            {solutions.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">暂无方案</td></tr>
            )}
          </tbody>
        </table>
      </div>
